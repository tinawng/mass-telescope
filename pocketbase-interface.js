export default class PocketBaseInterface {
    ...
    async auth(identity, password) {
        ...
        const responde_json = await response.json() /**💡 All api responses are expected to return json */

        if (response.ok) {
            this.fetch_headers["Authorization"] = responde_json.token
            return
        }
        throw responde_json
    }
    ...
    async create(body) {
        ...
        if (response.ok) return responde_json
        throw responde_json
    }
    /**
    ...
    async getOne(id, args = {}) {
        ...
        if (response.ok) return this._keysFilter(responde_json, args.keys)
        throw responde_json
    }
    ...
    async getList(args = {}, page_length = 50) {
        if (args.page_index) {
            ...
            if (response.ok) {
                const records = responde_json.items
    ...
    async getCount(filter) {
        ...
        if (response.ok) return responde_json.totalItems
        throw responde_json
    }
